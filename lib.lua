...
local b = Color3.new(255,255,255)
local speedrainbow = 0.5


local a = Color3.fromRGB(255,255,255)
local tc = Color3.fromRGB(50, 50, 50)
local bg = a
local tb = a
local sd = a
local nbg = a
...
local RayfieldLibrary = {
	...
	Theme = {
        ...
        Rainbow = {
			...
			ToggleDisabled = Color3.fromRGB(100, 100, 100),
			ToggleEnabledStroke = Color3.fromRGB(0, 170, 255),
			ToggleDisabledStroke = b,
			ToggleEnabledOuterStroke = Color3.fromRGB(100, 100, 100),
			ToggleDisabledOuterStroke = b,
		...
		Custom = {
			TextFont = "Gotham", -- Default will use the various font faces used across Rayfield
			TextColor = tc, -- i need to make all text 240, 240, 240 and base gray on transparency not color to do this
			
			Background = bg,
			Topbar = tb,
			Shadow = sd,

			NotificationBackground = nbg,
			NotificationActionsBackground = nabg,

			TabBackground = tbg,
			TabStroke = ts,
			TabBackgroundSelected = tbgs,
			TabTextColor = ttb,
			SelectedTabTextColor = sttc,
		
			ElementBackground = eb,
			ElementBackgroundHover = ebh,
			SecondaryElementBackground = sebg, -- For labels and paragraphs
			ElementStroke = es,
			SecondaryElementStroke = ses, -- For labels and paragraphs

			SliderBackground = sbg,
			SliderProgress = sp,
			SliderStroke = ss,

			ToggleBackground = tbg,
			ToggleEnabled = te,
			ToggleDisabled = td,
			ToggleEnabledStroke = tes,
			ToggleDisabledStroke = tds,
			ToggleEnabledOuterStroke = teos,
			ToggleDisabledOuterStroke = tdos,

			InputBackground = ibg,
			InputStroke = is,
			PlaceholderColor = phc
		}
	}
}
...
local Rayfield = game:GetObjects("rbxassetid://12218567306")[1]
...
function ChangeTheme(ThemeName)
	...
	for _, obj in ipairs(Rayfield:GetDescendants()) do
		if obj.ClassName == "TextLabel" or obj.ClassName == "TextBox" or obj.ClassName == "TextButton" then
            ...
            else obj.TextColor3 = Color3.fromRGB(240, 240, 240)
			end
		end
	end
...
------------------------------------------------------------------------------
			ColorPicker.Button.Position = UDim2.new(0, 135, 0, 90)
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				if not opened then
					...
					TweenService:Create(ColorPicker.Button, TweenInfo.new(0.5, Enum.EasingStyle.Quint), {Position = UDim2.new(0, 135, 0, 73)}):Play()
				else
					opened = false
					...
					TweenService:Create(ColorPicker.Button, TweenInfo.new(0.5, Enum.EasingStyle.Quint), {Position = UDim2.new(0, 135, 0, 90)}):Play()
				end
			end)
			...
			ColorPicker.RGB.RInput.InputBox.FocusLost:connect(function()
				rgbBoxes(ColorPicker.RGB.RInput.InputBox.Text,"R")
				pcall(function()ColorPickerSettings.Callback(Color3.fromHSV(h,s,v))end)
			end)
			ColorPicker.RGB.GInput.InputBox.FocusLost:connect(function()
				rgbBoxes(ColorPicker.RGB.GInput.InputBox.Text,"G")
				pcall(function()ColorPickerSettings.Callback(Color3.fromHSV(h,s,v))end)
			end)
			ColorPicker.RGB.BInput.InputBox.FocusLost:connect(function()
				rgbBoxes(ColorPicker.RGB.BInput.InputBox.Text,"B")
				pcall(function()ColorPickerSettings.Callback(Color3.fromHSV(h,s,v))end)
			end)
            local rainbowt = false
            local d
            Button.Interact.MouseButton1Click:Connect(function()
if rainbowt == false then
        rainbowt = true
else rainbowt = false
end	
...
local num = 0.05
local num = 0
function speedrain(speedrainbownew,unu)
speedrainbow = speedrainbownew
if speedrainbownew == 100 then speedrainbow = 0 end
if unu>12 then num = 0.01+unu/1000 end
if unu<12 then num = 0.01 end
end
...
while rainbowt do
 counter = counter + num
 d = Color3.fromHSV(zigzag(counter),1,1)
wait(speedrainbow)
pcall(function()ColorPickerSettings.Callback(d)end)
local hex = d:ToHex(R)
